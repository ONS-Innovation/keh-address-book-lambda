...
class S3Writer:
    ...
    def write_data_to_s3(self, file_to_update: str, data: str):
        ...
        if file_to_update is None or data is None:
            self.logger.warning(f"filename or data is empty. filename: {'empty' if file_to_update is None else 'filled'}, data: {'empty' if data is None else 'filled'}")
        ...
        try:
            ...
            self.logger.log_error(f"Unable to upload updated username and email data to S3, {error}")
        else:
            self.logger.log_info("Successfully uploaded updated username and email data to S3")

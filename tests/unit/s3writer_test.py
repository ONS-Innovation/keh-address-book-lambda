...
def test_writes_to_s3_success(logger_spy):
    ...
    assert any(
        "Successfully uploaded updated username and email data to S3" in m
        for m in logger_spy.infos
    )
...
def test_writes_to_s3_error_logs(logger_spy):
    ...
    writer = S3Writer(
        logger=logger_spy, s3_client=FailingS3Client(), bucket_name="my-bucket"
    )
    ...
    assert any(
        "Unable to upload updated username and email data to S3" in m
        for m in logger_spy.errors
    )
...
def test_write_data_when_none(logger_spy):
    ...
    writer = S3Writer(
        logger=logger_spy, s3_client=GuardClient(), bucket_name="my-bucket"
    )
